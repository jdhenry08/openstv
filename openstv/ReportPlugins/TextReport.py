...
class TextReport(ReportPlugin):
    ...
    def generateWithdrawnText(self):
        ...
        if len(self.dirtyB.withdrawn) == 0:
            ...
            withdrawnText = "Removed withdrawn candidate %s from the ballots." % self.dirtyB.withdrawn[0]
        else:
            self.dirtyB.withdrawn.sort()
            withdrawnText = "Removed withdrawn candidates %s from the ballots." % self.dirtyB.joinList(
                self.dirtyB.withdrawn
            )
...
""" % (
                self.generateMatrix(self.e.dMat),
                self.e.SSDinfo,
            )
            ...
            self.output("Using the Borda count to choose the winner " "from the Smith set.\n\n")
            R = TextReport(self.e.e, self.maxWidth, "table", outputFile=self.outputFile)
            R.generateReport()
            self.output("\n")
    ...
    def generateTextRoundResults(self, R, width, nSubCol):
        ...
        ## Print message.
        self.output("  |" + "-" * (width - 3) + "\n")
        line = textwrap.fill(self.e.msg[R], initial_indent="  | ", subsequent_indent="  | ", width=width)
        self.output(line + "\n")
    ...
    def generateReportNonIterative(self):
        ...
        for c in range(self.cleanB.numCandidates):
            out((fmt1 + " | " + fmt2 + "\n") % (self.cleanB.names[c], self.e.displayValue(self.e.count[c])))
