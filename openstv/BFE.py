...
class BFEFrame(wx.Frame):
    ...
    def __init__(self, parent, mode):
        ...
        # Edit an existing ballot file
            ...
            dlg = wx.FileDialog(self, "Edit Ballot File", style=wx.OPEN | wx.CHANGE_DIR)
            if dlg.ShowModal() != wx.ID_OK:
                dlg.Destroy()
                self.Destroy()
                return
        ...
        self.log = wx.TextCtrl(nb, -1, style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_WORDWRAP | wx.FIXED)
        ...
        if mode == "new":
            ...
            self.Log("Loaded %d ballots from file %s." % (self.b.numBallots, os.path.basename(self.b.getFileName())))
        else:
            assert 0
    ...
    def OnAppendBF(self, event):
        ...
        dlg = wx.FileDialog(self, "Select Ballot File", style=wx.OPEN | wx.CHANGE_DIR)
        if dlg.ShowModal() != wx.ID_OK:
            dlg.Destroy()
            return
        fName = dlg.GetPath()
        ...
        try:
            ...
            self.Log("Appended %d ballots from file %s." % (self.b.numBallots - oldNumBallots, os.path.basename(fName)))
    ...
    def OnSaveBallotsAs(self, event):
        ...
        dlg = wx.FileDialog(self, "Save Ballot File", style=wx.SAVE | wx.OVERWRITE_PROMPT | wx.CHANGE_DIR)
        if dlg.ShowModal() != wx.ID_OK:
            dlg.Destroy()
            return
        fName = dlg.GetPath()
    ...
    def OnSaveLogAs(self, event):
        dlg = wx.FileDialog(self, "Save Log to a File", style=wx.SAVE | wx.OVERWRITE_PROMPT | wx.CHANGE_DIR)
        if dlg.ShowModal() != wx.ID_OK:
            dlg.Destroy()
            return
    ...
    def OnCloseWindow(self, event):
        ...
        if self.panel.NeedToSaveBallots == True:
            ...
            saveBallots = wx.MessageBox(msg, "Warning", wx.YES_NO | wx.CANCEL | wx.ICON_INFORMATION)
        ...
        if self.panel.NeedToSaveLog == True:
            msg = "Do you want to save the log for ballot file %s?" % os.path.basename(self.b.getFileName())
            saveLog = wx.MessageBox(msg, "Warning", wx.YES_NO | wx.CANCEL | wx.ICON_INFORMATION)
...
remove the ranking and reorder the remaining candidates."""
        ...
        w = (
            self.ballotC.GetColumnWidth(0)
            + self.ballotC.GetColumnWidth(1)
    ...
    def UpdatePanel(self):
        ...
        for ranking in self.currentBallot:
            if isinstance(ranking, list):
                ...
                break
        self.currentBallot = [r for r in self.currentBallot if r != -1 and not isinstance(r, list)]

        # Update the list box to show the current ballot
        for c in range(self.b.numCandidates):
            if self.currentBallot.count(c) > 1:
                warnings.warn(
                    "Candidate %s appears on ballot %d more than once.  Later rankings ignored."
    ...
    def OnListClick(self, event):
        ...
        if c not in self.currentBallot:
            ...
            self.Log("Added candidate %s to ballot %d with rank %d." % (name, self.i + 1, rank))
    ...
    def OnListDClick(self, event):
        ...
        if c in self.currentBallot:
            ...
            self.storeCurrentBallot()
            self.UpdatePanel()
            self.Log("Removed candidate %s from ballot %d." % (name, self.i + 1))
...
name below.""",
    ...
    def OnOK(self, event):
        ...
        if name == "":
            ...
            wx.MessageBox(
                "Name entered but not added.  Please hit the 'Add' "
    ...
    def OnAdd(self, event):
        ...
        if name in self.names:
            wx.MessageBox("Can't have two candidates with the same name.", "Error", wx.OK | wx.ICON_ERROR)
            self.candidateC.SetFocus()
            return
        # Make sure the name is not empty
        if name == "":
            wx.MessageBox("Candidate name cannot be blank.", "Error", wx.OK | wx.ICON_ERROR)
            self.candidateC.SetFocus()
            return
