...
class ElectionMethod(object):
    ...
    def preCount(self):
        ...
        assert self.strongTieBreakMethod in ["random", "alpha", "index", "manual"]
...
Need at least %d candidates but have only %d."""
    ...
    def chooseNfromM(self, N, values, candidateList, what):
        ...
        if len(chosen) < N:
            maybe.sort()
            desc = "Candidates %s were tied when when choosing %s. " % (self.b.joinList(maybe), what)
        while len(chosen) < N:
    ...
    def breakStrongTie(self, tiedCandidates, what=""):
        ...
        # Break the tie randomly.
            ...
            desc = "Candidate %s was chosen by breaking the tie randomly. " % self.b.names[c]
        ...
        # Break the tie alphabetically by candidate's names.
            ...
            c = tiedCandidates[0]
            desc = "Candidate %s was chosen by breaking the tie alphabetically. " % self.b.names[c]

        # Break the tie by candidate index number.
        elif self.strongTieBreakMethod == "index":
            tiedCandidates.sort()
            c = tiedCandidates[0]
            desc = "Candidate %s was chosen by breaking the tie by candidate index " "number. " % self.b.names[c]

        elif self.strongTieBreakMethod == "manual":
            self.breakTieRequestQueue.put([tiedCandidates, [self.b.names[c] for c in tiedCandidates], what])
            c = self.breakTieResponseQueue.get(True)
            if c == None:
                c = random.choice(tiedCandidates)
                desc = "Candidate %s was chosen by breaking the tie randomly. " % self.b.names[c]
            else:
                desc = "Candidate %s was chosen by breaking the tie manually. " % self.b.names[c]
...
class NonIterative(ElectionMethod):
    ...
    def chooseWinners(self):
        ...
        (winners, desc) = self.chooseNfromM(self.numSeats, self.count, self.continuing, "winner")
...
class Iterative(ElectionMethod):
    ...
    def breakWeakTie(self, R, candidateList, mostfewest, what=""):
        ...
        tiedCandidates.sort()
        desc = "Candidates %s were tied when choosing %s. " % (self.b.joinList(tiedCandidates), what)
    ...
    def newWinners(self, newWinnersList, status="over"):
        ...
        if len(newWinnersList) == 1 and status == "over":
            desc = "Candidate %s has reached the threshold and is elected. " % self.b.joinList(newWinnersList)
        elif len(newWinnersList) == 1 and status == "under":
            desc = "Candidate %s is elected. " % self.b.joinList(newWinnersList)
        elif status == "over":
            desc = "Candidates %s have reached the threshold and are elected. " % self.b.joinList(newWinnersList)
        elif status == "under":
    ...
    def electionOver(self):
        ...
        if "Know Winners" in self.stopCond and len(self.winners) == self.numSeats:
            return True

        # Election is over when N+1 or fewer candidates remain
        if "N+1" in self.stopCond and len(self.continuing) + len(self.winners) <= self.numSeats + 1:
            return True

        # Election is over when fewer than N candidates remain
        if "N" in self.stopCond and len(self.continuing) + len(self.winners) <= self.numSeats:
            return True
...
class STV(Iterative):
    ...
    def isSurplusToTransfer(self):
        ...
        if self.surplus[self.R - 1] == 0 or (self.delayedTransfer == "On" and len(self.getSureLosers()) != 0):
            return False
        else:
            return True
    ...
    def selectSurplusToTransfer(self):
        ...
        (c, desc) = self.breakWeakTie(self.R - 1, list(self.winnersOver), "most", "surplus to transfer")
        return (c, desc)
    ...
    def getSureLosers(self, R=None):
        ...
        for i, cluster in enumerate(clusteredContinuing[:-1]):
            ...
            if (
                ...
                or (self.batchElimination == "LosersERS97" and s <= nextClusterCount)
    ...
    def selectCandidatesToEliminate(self):
        ...
        if self.firstEliminationRound:
            ...
            if self.batchElimination == "Zero":
                desc = "Since this is the first elimination round, all candidates " "without any votes are eliminated. "
                elimList = [c for c in self.continuing if self.count[self.R - 1][c] == 0]
            elif self.batchElimination == "Cutoff":
                desc = (
                    "Since this is the first elimination round, all candidates "
                    "with fewer than %d votes are eliminated. " % self.batchCutoff
                )
                elimList = [c for c in self.continuing if self.count[self.R - 1][c] < self.p * self.batchCutoff]
                ...
                if self.surplus[self.R - 1] == 0:
                    ...
                    if len(ctng) + len(self.winners) > self.numSeats:
                        ...
                        if elimListTotalCount == 0:
                            (c, desc2) = self.breakWeakTie(self.R - 1, ctng, "fewest", "candidates to eliminate")
                            elimList.append(c)
                            desc += desc2
        ...
        if (not self.firstEliminationRound) or (len(elimList) == 0):
            if self.batchElimination in ["Losers", "LosersERS97"]:
                ...
                elimList = self.getSureLosers()
                if len(elimList) == 0:
                    (c, desc2) = self.breakWeakTie(
                        self.R - 1, list(self.continuing), "fewest", "candidates to eliminate"
                    )
                    elimList = [c]
                    desc += desc2
            else:
                (c, desc) = self.breakWeakTie(self.R - 1, self.continuing, "fewest", "candidates to eliminate")
                elimList = [c]
    ...
    def updateCandidateStatus(self):
        ...
        if len(self.winners) == self.numSeats:
            ...
            if len(self.continuing) + len(self.winners) > self.numSeats:
                (c, desc) = self.breakWeakTie(self.R, self.continuing, "fewest", "winners")
                self.newLosers([c])
...
class GregorySTV(OrderIndependentSTV):
    ...
    def transferSurplusVotesFromCandidate(self, cSurplus):
        ...
        nTransferable = 0
        for i in lastBatch:
            if self.b.getTopChoiceFromWeightedBallot(i, self.continuing) is not None:
                transferableValue += self.b.getWeight(i) * self.transferValue[i]
    ...
    def updateCount(self):
        ...
        for c in self.losers | self.continuing | self.winnersOver:
            ...
            for i in self.votes[c]:
                self.count[self.R][c] += self.b.getWeight(i) * self.transferValue[i]
...
class WeightedInclusiveSTV(OrderIndependentSTV):
    ...
    def transferSurplusVotesFromCandidate(self, cSurplus):
        ...
        for i in self.votes[cSurplus][:]:
            self.transferValue[i] = self.transferValue[i] * surplus / self.count[self.R - 1][cSurplus]
            c = self.b.getTopChoiceFromWeightedBallot(i, self.continuing)
            if c is not None:
                self.votes[c].append(i)
        ...
        desc = "Count after transferring surplus votes from %s with a transfer " "value of %s/%s. " % (
...
class RecursiveSTV(OrderIndependentSTV):
    ...
    def isSurplusToTransfer(self):
        ...
        if (
            self.surplus[self.R - 1] < self.surplusLimit
            or (self.delayedTransfer == "On" and len(self.getSureLosers()) != 0)
            or self.inInfiniteLoop()
        ):
            return False
        else:
            return True
    ...
    def transferSurplusVotes(self):
        ...
        self.roundInfo[self.R]["surplus"] = "Count after transferring surplus votes. " + desc
    ...
    def updateKeepFactors(self):
        ...
        for c in candidateList:
            if self.count[self.R - 1][c] > self.thresh[self.R - 1]:
                ...
                kf, rem = divmod(self.keepFactor[self.R - 1][c] * self.thresh[self.R - 1], self.count[self.R - 1][c])
                if rem > 0:
                    kf += 1
                self.keepFactor[self.R][c] = kf
                winners.append("%s, %s" % (self.b.names[c], self.displayValue(self.keepFactor[self.R][c])))
            else:
                self.keepFactor[self.R][c] = self.keepFactor[self.R - 1][c]
