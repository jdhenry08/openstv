...
class Ballots(object):
    ...
    def getSortedWeightedBallots(self):
        ...
        sortedBallots = [
            (str(self.uniqueBallots[i]), self.uniqueBallotCount[i]) for i in range(self.numWeightedBallots)
        ]
        sortedBallots.sort()
    ...
    def getCleanBallots(self, removeEmpty=True, removeOvervotes="Cambridge", removeDupes=True, removeWithdrawn=True):
        ...
        cleanBallots.names = [self.names[c] for c in range(self.numCandidates) if c not in self.withdrawn]
    ...
    def appendFile(self, fName):
        ...
        if (
            ballotList.numSeats != self.numSeats
            or ballotList.names != self.names
            or ballotList.withdrawn != self.withdrawn
        ):
            raise RuntimeError(
