...
"""
    ...
    def selectCandidatesToEliminate(self):
        ...
        if losers == [] and self.surplus[self.R - 1] < self.surplusLimit:
            (c, desc) = self.breakWeakTie(self.R - 1, ctng, "fewest", "candidates to eliminate")
            losers = [c]
        ...
        if losers == [] and self.inInfiniteLoop():
            ...
            (c, desc2) = self.breakWeakTie(self.R - 1, self.continuing, "fewest", "candidates to eliminate")
            losers = [c]
            desc += desc2
        ...
        for c in candidateList:
            if self.count[self.R - 1][c] > self.thresh[self.R - 1]:
                ...
                if method == "nz":
                    ...
                    kf, rem = divmod(
                        self.keepFactor[self.R - 1][c] * self.thresh[self.R - 1], self.count[self.R - 1][c]
                    )
                    if rem > 0:
                        kf += 1
    ...
    def describeRound(self):
        ...
        elif "surplus" in self.roundInfo[self.R] and "eliminate" in self.roundInfo[self.R]:
            text = self.roundInfo[self.R]["eliminate"] + self.roundInfo[self.R]["surplus"]
