...
"""
    ...
    def preCount(self):
        ...
        assert self.completion in ["Schwartz Sequential Dropping", "IRV on Smith Set", "Borda on Smith Set"]
    ...
    def computeSmithSet(self):
        ...
        for c in range(self.b.numCandidates):
            for d in range(self.b.numCandidates):
                if c != d:
                    if (not dMat[c][d]) and dMat[d][c] and (c in self.smithSet):
...
Tie broken randomly.""" % self.b.joinList(
                ctng
            )
