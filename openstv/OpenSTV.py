...
class Election:
    ...
    def runElection(self):
        ...
        while countThread.is_alive():
            ...
            if "R" in vars(self.e):
                status = "Counting votes using %s\nRound: %d" % (self.e.longMethodName, self.e.R + 1)
            else:
                status = "Counting votes using %s\nInitializing..." % self.e.longMethodName
            dlg.Pulse(status)
        dlg.Destroy()
...
to break the tie randomly."""
            % what
        )
        dlg = wx.SingleChoiceDialog(self.frame, instructions, "Break Tie Manually", names, wx.CHOICEDLG_STYLE)
...
class Frame(wx.Frame):
    ...
    def __init__(self, parent):
        ...
        self.console = wx.TextCtrl(
            self.notebook, -1, style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_WORDWRAP | wx.TE_RICH2
        )
...
"""
    ...
    def MakeMenu(self):
        ...
        # File menu
        FileMenu = wx.Menu()
        self.AddMenuItem(FileMenu, "Run Election...\tCtrl+E", "Run election...", self.OnRunElection)
        FileMenu.AppendSeparator()
        self.AddMenuItem(FileMenu, "New Ballot File...\tCtrl+N", "New ballot file...", self.OnNewBF)
        self.AddMenuItem(FileMenu, "Edit Ballot File...\tCtrl+O", "Edit ballot file...", self.OnEditBF)
        FileMenu.AppendSeparator()
        self.AddMenuItem(FileMenu, "Close Tab\tCtrl+W", "Close Tab", self.OnCloseTab)
        FileMenu.AppendSeparator()
        self.AddMenuItem(FileMenu, "Save Results As CSV...", "Save results as CSV", self.OnSaveResultsCSV)
        self.AddMenuItem(FileMenu, "Save Results As Text...", "Save results as text", self.OnSaveResultsText)
        self.AddMenuItem(FileMenu, "Save Results As HTML...", "Save results as HTML", self.OnSaveResultsHTML)
        menuId = self.AddMenuItem(FileMenu, "Exit", "Exit the application", self.OnExit, "Exit")
        ...
        self.AddMenuItem(EditMenu, "Select All\tCtrl+A", "Select all", self.OnSelectAll)
        self.menuBar.Append(EditMenu, "&Edit")

        # Options menu
        OptionsMenu = wx.Menu()
        self.AddMenuItem(OptionsMenu, "Show All Methods", "Show all methods", self.OnShowAll, "Check")
        self.AddMenuItem(OptionsMenu, "Break Ties Randomly", "Break Ties Randomly", self.OnBreakTiesRandomly, "Check")
        subMenu = wx.Menu()
        ...
        self.AddMenuItem(HelpMenu, "OpenSTV Help", "OpenSTV Help", self.OnHelp, "Help")

        self.AddMenuItem(HelpMenu, "License", "GNU General Public License", self.OnLicense)
        ...
        if wx.Platform != "__WXMAC__":
            HelpMenu.AppendSeparator()
        itemId = self.AddMenuItem(HelpMenu, "&About", "About OpenSTV", self.OnAbout, "About")
        if wx.Platform == "__WXMAC__":
    ...
    def OnRunElection(self, event):
        ...
        tc = wx.TextCtrl(self.notebook, -1, style=wx.TE_MULTILINE | wx.TE_READONLY | wx.HSCROLL)
        tc.SetMaxLength(0)
    ...
    def OnSaveResultsCSV(self, event):
        ...
        n = self.notebook.GetSelection()
        if n == 0:
            wx.MessageBox("Please select a tab containing election results.", "Message", wx.OK | wx.ICON_INFORMATION)
            return
        T = self.electionList[n - 1]
        if T.e.methodName == "Condorcet":
            wx.MessageBox("CSV report not available for Condorcet elections.", "Message", wx.OK | wx.ICON_INFORMATION)
            return

        if T.e.methodName == "Condorcet":
            wx.MessageBox("Not available for this method.", "Message", wx.OK | wx.ICON_INFORMATION)
            return

        dlg = wx.FileDialog(
            self, "Save Results in CSV Format", style=wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT | wx.FD_CHANGE_DIR
        )
        if dlg.ShowModal() != wx.ID_OK:
            dlg.Destroy()
            return
    ...
    def OnSaveResultsText(self, event):
        ...
        dlg = wx.FileDialog(
            self, "Save Results in Text Format", style=wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT | wx.FD_CHANGE_DIR
        )
        if dlg.ShowModal() != wx.ID_OK:
            dlg.Destroy()
            return
    ...
    def OnSaveResultsHTML(self, event):
        ...
        T = self.electionList[n - 1]
        if T.e.methodName == "Condorcet":
            wx.MessageBox("HTML report not available for Condorcet elections.", "Message", wx.OK | wx.ICON_INFORMATION)
            return

        dlg = wx.FileDialog(
            self, "Save Results in HTML Format", style=wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT | wx.FD_CHANGE_DIR
        )
        if dlg.ShowModal() != wx.ID_OK:
            dlg.Destroy()
            return
    ...
    def OnLicense(self, event):
        frame = HTMLFrame(self, "GNU General Public License", fName="License.html")
        frame.Show(True)
...
See the Help menu for more information about the available methods.""",
        ...
        self.withdrawC = wx.CheckBox(self, -1, "Withdraw canddiates:", style=wx.ALIGN_RIGHT)
    ...
    def OnFilenameSelect(self, event):
        dlg = wx.FileDialog(self, "Select Input File", "", style=wx.FD_OPEN | wx.FD_CHANGE_DIR)
        if dlg.ShowModal() != wx.ID_OK:
            dlg.Destroy()
            return
    ...
    def OnOK(self, event):
        ...
        if filename == "":
            wx.MessageBox("Please select a filename.", "Message", wx.OK | wx.ICON_INFORMATION)
            return
...
""",
        ...
        self.Bind(wx.EVT_LIST_ITEM_ACTIVATED, self.OnListDClick, self.withdrawC)
...
class ElectionOptionsDialog(wx.Dialog):
    ...
    def __init__(self, parent, election):
        ...
        file2L = wx.StaticText(self, -1, os.path.basename(self.e.dirtyBallots.getFileName()))
        ...
        for option in self.e.e.guiOptions:
            ...
            labelList.append(label)  # type: ignore
            self.ctrlList.append(control)  # type: ignore
